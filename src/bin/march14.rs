...
fn render(pos: Vec3, dir: Vec3) -> Color {
    ...
    match march(pos, dir, 300, 0.5) {
        Some(hit) => {
            ...
            let sun_fac = (0..10)
                .map(|_| {
                    let random = xyz(rand::random(), rand::random(), rand::random()) * 0.04;
                    if march(hit.point + hit.normal * 0.04, light_dir + random, 100, 0.5).is_none()
                    {
                        1.0
                    } else {
                        0.0
...
fn main() {
    ...
    canvas.render(move |camera, img| {
        ...
        img.as_mut()[w * i..w * (i + 8)]
            .par_chunks_mut(w)
            .enumerate()
            .for_each(|(y, row)| {
                let y = ((y + i) as f32).remap(0.0..h as f32, -1.0..1.0);
